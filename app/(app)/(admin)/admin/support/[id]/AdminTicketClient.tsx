...
export default function AdminTicketClient({
  ...
  const otherTyping =
    typingUsers[ticketId]?.filter((id) => id !== user?._id) || [];
      ...
      <PageHeader
        ...
        actions={
          <div className="flex items-center gap-2 text-xs bg-white/5 border border-white/10 rounded-lg px-3 py-1.5">
            <span
              className={`w-2 h-2 rounded-full ${isConnected ? "bg-emerald-500" : "bg-yellow-500"}`}
            />
              ...
              <div className="flex items-center gap-2">
                ...
                {ticket.status !== "closed" && (
                  <Select
                    ...
                    classNames={{
                      trigger:
                        "h-7 text-xs border-white/10 bg-white/5 text-white w-full",
                      popoverContent:
