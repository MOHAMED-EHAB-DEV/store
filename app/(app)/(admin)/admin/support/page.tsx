...
async function getSupportTickets(searchParams: {
  ...
  try {
    const response = await fetch(
      `${process.env.NEXT_PUBLIC_APP_URL}/api/admin/tickets?${params.toString()}`,
  ...
  if (!data) {
    return (
      <div className="p-6 text-center">
        <ErrorState message="Failed to load support tickets. Please try again." />
      </div>
    );
  }
