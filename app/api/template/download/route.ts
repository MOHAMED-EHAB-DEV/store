  ...
  try {
    ...
    if (template.type !== "coded") {
      return createErrorResponse(
        "Download not available for this template type",
        403,
      );
    }
    ...
    if (template.price > 0) {
      ...
      if (!purchased)
        return createErrorResponse(
          "You must purchase this template to download it",
          403,
        );
    }
    // else {
    ...
    if (!fileKey)
      return createErrorResponse("Template file missing from storage", 500);

    const { url: signedUrl, token } = await getGoogleDriveDownloadUrlAndToken(
      fileKey as string,
    );
    if (!signedUrl || !token) {
      console.error("Failed to generate download token for fileKey:", fileKey);
    ...
    if (!upstreamRes.ok || !upstreamRes.body) {
      if (upstreamRes.status === 416)
        return createErrorResponse("Requested range not satisfiable", 416);
    ...
    if (upstreamAcceptRanges)
      resHeaders.set("Accept-Ranges", upstreamAcceptRanges);
    const upstreamContentRange = upstreamRes.headers.get("content-range");
    if (upstreamContentRange)
      resHeaders.set("Content-Range", upstreamContentRange);

    // Filename derived only from title
    const rawTitle =
      (template.title && String(template.title).trim()) ||
      (template.title && String(template.title).trim()) ||
      "download";
    ...
    resHeaders.set(
      "Content-Disposition",
      `attachment; filename="${suggestedFilename.replace(/"/g, "")}"; filename*=UTF-8''${encodedName}`,
    );
