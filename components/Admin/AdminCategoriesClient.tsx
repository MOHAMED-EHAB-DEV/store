...
export default function AdminCategoriesClient({
  ...
  const handleRefreshCounts = async () => {
    ...
    try {
      const res = await fetch("/api/admin/categories/refresh-counts", {
        method: "POST",
      });
      const data = await res.json();
      if (data.success) {
        sonnerToast.success(
          data.message || "All template counts refreshed successfully",
        );
        router.refresh();
      } else {
        sonnerToast.error(data.message || "Failed to refresh template counts");
      }
    } catch (error) {
      sonnerToast.error("An error occurred. Please try again.");
    } finally {
  ...
  const handleBulkStatusChange = async (isActive: boolean) => {
    ...
    try {
      ...
      if (data.success) {
        sonnerToast.success(
          `${selectedIds.length} categories updated successfully`,
        );
        setSelectedIds([]);
        router.refresh();
      } else {
