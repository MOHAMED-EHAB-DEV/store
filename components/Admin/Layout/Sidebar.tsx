                ...
                <div className="flex flex-col gap-1 md:mt-14 mt-10">
                    {AdminSidebarLinks.map(({ Icon, text, link }, idx) => (
                        <div key={idx} onClick={() => {
                            ...
                            className={`w-full h-10 cursor-pointer transition-all px-5 py-3 rounded-md flex gap-3 items-center ${link === path ? "bg-white/10" : "hover:bg-white/10"}`}>
                            <Icon className="w-5 h-5 text-white" />
                            <span className={`text-white text-sm ${link === path ? "font-bold" : ""}`}>{text}</span>
                        </div>
                    ))}
                </div>
            </div>
