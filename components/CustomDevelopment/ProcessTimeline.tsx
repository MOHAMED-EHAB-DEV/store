  ...
  useGSAP(
    () => {
      ...
      if (lineMobile) {
        tl.to(
          lineMobile,
          { height: "100%", duration: totalDuration, ease: "none" },
          0,
        );
      }
  ...
  return (
    <section ref={containerRef} className="py-24 px-4 w-full max-w-7xl mx-auto">
      <div className="text-center mb-24">
        <h2 className="text-3xl md:text-5xl font-bold mb-4 font-paras">
          Our Process
        </h2>
        <p className="text-gray-400 max-w-2xl mx-auto">
          A streamlined, transparent workflow designed to deliver results
          efficiently.
        </p>
      </div>
      ...
      <div className="relative">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 md:gap-4 relative z-10">
          {STEPS.map((step, idx) => (
            <div
              key={idx}
              className="timeline-step flex flex-col items-center md:items-start relative group"
            >
              <div className="w-16 h-16 rounded-full bg-surface-dark border-2 border-white/20 flex items-center justify-center text-xl font-bold text-white group-hover:border-purple-500 transition-colors mb-6 z-10">
                {step.number}
              </div>
