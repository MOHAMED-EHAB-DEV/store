  ...
  const validateFileSecurity = async (file: File): Promise<boolean> => {
    ...
    if (parts.length > 2) {
      ...
      if (suspiciousExts.some((ext) => parts.includes(ext.toLowerCase()))) {
        toast.error(
          `Security alert: Suspicious double extension in ${file.name}`,
        );
        return false;
      }
    }
      ...
      reader.onloadend = (e) => {
        ...
        if (
          ...
          if (
            ...
            toast.error(
              `Security alert: Invalid ZIP signature in ${file.name}. File spoofing detected.`,
            );
            resolve(false);
            return;
          }
        ...
        if (
          ...
          if (binarySignatures.some((sig) => header.startsWith(sig))) {
            toast.error(
              `Security alert: Text file ${file.name} contains binary signatures.`,
            );
            resolve(false);
            return;
          }
      ...
      if (
        ...
        fullReader.onloadend = (e) => {
          ...
          for (let i = 0; i < scanLength; i++) {
            if (textArr[i] === 0) {
              toast.error(
                `Security alert: Binary content detected in text file ${file.name}.`,
              );
              resolve(false);
              return;
            }
  ...
  const processFiles = async (fileList: File[]) => {
    ...
    for (const file of fileList) {
      ...
      if (!isImage && !isZip && !isTxt) {
        toast.error(
          `File type not allowed for ${file.name}. Only images, zip, and txt are permitted.`,
        );
        continue;
      }
