...
const BackToTop = () => {
    ...
    return (
        <div className="relative mt-4 group w-fit">
            <div aria-hidden="true" className="absolute inset-0 bg-gradient-to-r from-purple-600 via-pink-500 to-orange-500 rounded-full blur-md opacity-50 group-hover:opacity-100 transition-opacity duration-300" />
            <Button
                variant="ghost"
                size="icon"
                onClick={scrollToTop}
            ...
            >
                <ChevronUp strokeWidth={3} className="w-5 h-5 transition-transform group-hover:-translate-y-1" aria-hidden="true" />
            </Button>
        </div>
    );
};
