  ...
  return (
    <header
        className={`navbar z-50 fixed top-2 left-1/2 -translate-x-1/2 w-[95%] max-w-7xl transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)] rounded-full ${
          scrolled
            ? isCustomDev
              ? "bg-black/40 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-white/10 py-3 mt-0"
              : "bg-primary/80 backdrop-blur-xl shadow-[0_8px_32px_rgba(0,0,0,0.5)] border border-white/10 py-3 mt-0"
            : isCustomDev
      ...
      >
        <div className="flex items-center justify-between px-6 md:px-8">
          ...
          <div className={`flex gap-2 ${!user && "flex-1 justify-end"}`}>
            ...
            {!user && (
              <div className="hidden sm:flex gap-2 items-center justify-end">
                ...
                <Link
                  ...
                  aria-label={
                    scrolled && !isCustomDev
                      ? "Work with Me - Request custom build"
            ...
            <Suspense fallback={
              <div className="relative p-2 rounded-xl opacity-50" aria-hidden="true">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-6 h-6 text-white" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
const NavbarItem = ({ text, link, isActive }: { text: string; link: string, isActive: Boolean }) => {
  return (
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="w-0 h-[2px] group-hover:w-full bg-gradient-to-r from-[#8BFA9E] to-[#3FD6DD] rounded-[2px] transition-all duration-300 ease-out"
      />
    </Link>
  );
};
