...
export function HorizontialMarquee(
    ...
    return (
        <div
            className={cn("w-full inline-flex flex-nowrap overflow-hidden [mask-image:_linear-gradient(to_right,transparent_0,_black_128px,_black_calc(100%-200px),transparent_100%)]", className)}>
            <div className={cn(animationClasses, `flex items-center gap-4 justify-center md:justify-start`)}>
                {[...items, ...items, ...items, ...items, ...items, ...items, ...items, ...items].map((testimonial, i) => (
                    <div
                        key={i}
                        className="testimonial-card flex flex-col justify-center items-center gap-5 w-80 h-64 glass p-6 rounded-2xl transition-all duration-500"
                    >
