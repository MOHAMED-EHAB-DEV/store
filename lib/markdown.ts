...
function remarkShiki() {
    return async (tree: Root) => {
        ...
        visit(tree, 'code', (node: any) => {
            ...
            // mutate node to be an HTML node so remarkRehype + rehypeStringify pass it through
            node.type = 'html';
    ...
    return (tree: any) => {
        visit(tree, 'element', (node: any) => {
            ...
            const addClass = (c: string) => {
                ...
                node.properties.className = Array.isArray(existing) ? [...existing, ...c.split(' ')] : [String(existing), ...c.split(' ')];
            };
            ...
            switch (node.tagName) {
                ...
                case 'code':
                    ...
                    addClass('inline-code bg-gray-800 text-pink-400 px-1.5 py-0.5 rounded text-sm');
                    break;
                default:
                    break;
            }
...
export async function mdToHtmlAndHeadings(content: string) {
    const file = await unified()
        ...
        .use(rehypeSlug)
        .use(rehypeAddClasses) // add classes/attributes server-side
